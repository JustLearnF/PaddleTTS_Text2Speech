...
class PredictorInterface {
  public:
	...
	virtual std::shared_ptr<Predictor> LoadModel(
...
class TPredictor : public PredictorInterface {
  public:
	bool Init(const std::string &AcousticModelPath,
		...
		acoustic_model_predictor_ =
			LoadModel(AcousticModelPath, cpuThreadNum);
		if (acoustic_model_predictor_ == nullptr) {
			return false;
		}
		std::cout << "load asmodel sucessfully" << std::endl;
		vocoder_predictor_ = LoadModel(VocoderPath, cpuThreadNum);
		if (vocoder_predictor_ == nullptr) {
			return false;
		}
		std::cout << "load vocmodel sucessfully" << std::endl;
	...
	std::shared_ptr<Predictor> LoadModel(
		const std::string &modelPath,
		int cpuThreadNum) override {
		if (modelPath.empty()) {
			return nullptr;
		}
		//设置model路径
		std::string prog_path = modelPath + "/model.pdmodel";
		std::string param_path = modelPath + "/model.pdiparams";
		// 设置Config
		Config config;
		config.SetModel(prog_path, param_path);
		config.SetCpuMathLibraryNumThreads(cpuThreadNum);
	...
	std::unique_ptr<const Tensor> GetAcousticModelOutput(
		...
		auto phones_names = acoustic_model_predictor_->GetInputNames();
		auto phones_handle = acoustic_model_predictor_->GetInputHandle(phones_names[0]);
		phones_handle->Reshape({static_cast<int64_t>((long)phones.size())});
		phones_handle->CopyFromCpu(phones.data());
		acoustic_model_predictor_->Run();

		// 获取输出Tensor
		auto am_output_names = acoustic_model_predictor_->GetOutputNames();
		auto am_output_handle = acoustic_model_predictor_->GetOutputHandle(am_output_names[0]);
		// 打印输出Tensor的shape
	...
	std::unique_ptr<const Tensor> GetVocoderOutput(
		...
		auto mel_names = vocoder_predictor_->GetInputNames();
		auto mel_handle = vocoder_predictor_->GetInputHandle(mel_names[0]);
		// [?, 80]
		auto dims = amOutput->shape();
		mel_handle->Reshape(dims);
		paddle_infer::PlaceType place;
		int size = 0;
		auto* am_output_data = amOutput->data<float>(&place,&size);
		mel_handle->CopyFromCpu(am_output_data);
		vocoder_predictor_->Run();

		// 获取输出Tensor
		auto voc_output_names = vocoder_predictor_->GetOutputNames();
		auto voc_output_handle = vocoder_predictor_->GetOutputHandle(voc_output_names[0]);
		// 打印输出Tensor的shape
	...
	void VocoderOutputToWav(
		...
		paddle_infer::PlaceType place;
		int size = 0;
		auto output_data = vocOutput->data<float>(&place,&size);
  ...
  protected:
	...
	std::shared_ptr<Predictor> acoustic_model_predictor_ = nullptr;
	std::shared_ptr<Predictor> vocoder_predictor_ = nullptr;
};

template <>
uint16_t TPredictor<int16_t>::GetWavAudioFormat() {
	return TPredictor::WAV_FORMAT_16BIT_PCM;
}

template <>
uint16_t TPredictor<float>::GetWavAudioFormat() {
	return TPredictor::WAV_FORMAT_32BIT_FLOAT;
}

// 保存 16-bit PCM 格式 WAV
template <>
void TPredictor<int16_t>::SaveFloatWav(float *floatWav, int64_t size) {
	wav_.resize(size);
...
template <>
void TPredictor<float>::SaveFloatWav(float *floatWav, int64_t size) {
	wav_.resize(size);
